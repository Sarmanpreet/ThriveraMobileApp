...
import { getMOPProductListResponse, getSaveLeavesResponse, getSaveMopResponse } from 'src/app/tabs/employee/mop-list/store/Mop.selectors';
import { GetLeaveApprovedList, GetLeavePendingList, GetLeaveRejectedList, GetMOPProductList, resetLeave, resetMOP, ResetMOPProductList, saveLeaves, saveMOPEntry } from 'src/app/tabs/employee/mop-list/store/Mop.actions';
import { getSubProductDDLListResponse } from 'src/app/tabs/employee/store/Employee.selectors';
import { GetLeaveattandenceStatus, GetNewttandenceStatus, GetOldAttandenceStatus, GetRFCApprovedList, GetRFCPendingList, GetRFCRejectedList, saveRFCEntry } from 'src/app/tabs/employee/rfc-list/store/Rfc.actions';
import { getleaveDDLResponse, getnewAttnStatusResponse, getOldAttnStatusResponse, getSaveRFCResponse } from 'src/app/tabs/employee/rfc-list/store/Rfc.selectors';
@Component({
  selector: 'app-leave-entry-modal',
  templateUrl: './leave-entry-modal.component.html',
  styleUrls: ['./leave-entry-modal.component.scss'],
})
export class LeaveEntryModalComponent implements OnInit, OnDestroy {
  today = new Date();
  MOPForm: FormGroup;
  MopData: any = {};
  feedbacks: any = [];
  ...
  ngOnInit() {
    this.store.dispatch(resetLeave({ payload: '' }));
    ...
    const SubNewSubscription = this.store.pipe(select(getleaveDDLResponse))
      .subscribe(
        (getResponse) => {
          if (getResponse) {
    ...
    const SubscriptionAttlog = this.store.pipe(select(getSaveLeavesResponse))
      .subscribe(
        (getResponse) => {
          debugger;
          if (getResponse) {
  ...
  formatDate(value: string) {
    this.MOPForm.patchValue({ StartDate: format(parseISO(value), 'MMM dd yyyy') });

    return format(parseISO(value), 'MMM dd yyyy');

  }
  formatendDate(value: string) {
    this.MOPForm.patchValue({ EndDate: format(parseISO(value), 'MMM dd yyyy') });
  ...
  IntialiseDll(ID) {
    ...
    this.store.dispatch(GetLeaveattandenceStatus({ payload: data }))
  }
  initForm() {
    ...
    this.MOPForm = this.formBuilder.group({
      ...
      LeaveTypeID: this.formBuilder.control('', [Validators.required]),
  ...
  async saveRFCEntry() {
    ...
    if (form.valid) {
      ...
      this.store.dispatch(saveLeaves({ payload: data }));
