...
import { GetCompetitionList } from '../mop-list/store/Mop.actions';
import { getCompetetitionListResponse } from '../mop-list/store/Mop.selectors';
...
export class CompetitionListComponent extends BasePageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    const Subscription = this.store.pipe(delay(1000)).pipe(select(getCompetetitionListResponse))
      .subscribe(
        (Response) => {
  ...
  async OpenModal(params: any = undefined) {
    ...
    const modal = await this.modalController.create({
      component: CompetitionEntryModalComponent,
      swipeToClose: true,
